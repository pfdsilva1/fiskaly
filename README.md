...
- Golang (v1.22+) — uses `http.ServeMux` path variables (`{device_id}`)
...
This project used GPT-5 assistance for unit tests, README generation, and some code completions. All design and core implementation decisions were authored by me.
...
```bash
curl -s http://localhost:8080/api/v0/health | jq
```

Create device (algorithm = RSA|ECDSA, label [optional] = string):

```bash
curl -s "http://localhost:8080/api/v0/devices?algorithm=RSA&label=MyRSA" | jq
```

List devices:

```bash
curl -s http://localhost:8080/api/v0/devices | jq
```

Sign data (POST raw body):

```bash
DEVICE_ID=<id>
curl -s -X POST --data-binary @- \
  http://localhost:8080/api/v0/devices/$DEVICE_ID/sign <<<'hello world' | jq
```

List signatures for a device:

```bash
curl -s http://localhost:8080/api/v0/devices/$DEVICE_ID/signatures | jq
```
